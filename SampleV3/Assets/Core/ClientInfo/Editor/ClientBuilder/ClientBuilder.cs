...
public class ClientBuilder : EditorWindow
{
    ...
    private void Build(ClientInfo.AssetBuildMode assetBuildMode)
    {
        ...
        if (isBuildClient)
        {
            var scenes = EditorBuildSettings.scenes.Select(x => x.path).ToArray();
            BuildPipeline.BuildPlayer(scenes,
                                      $"{Application.dataPath.Replace("Assets", "")}Builds/{currentClientVersion}/{currentEnviroment}/{UnityEditor.EditorUserBuildSettings.activeBuildTarget}/{GetAssetTypeAsString(assetBuildMode)}/{currentEnviroment}/{Application.productName}{GetFileExtension()}",
                                      EditorUserBuildSettings.activeBuildTarget,
                                      GetBuildOptions());
        }
    }
