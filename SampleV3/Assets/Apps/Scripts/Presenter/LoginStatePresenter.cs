...
public class LoginStatePresenter : IDisposable
{
    ...
    private UIPopupOTP uiPopupOtp;


    private ISocketClient client;
    public ISocketClient Client => client ??= container.Inject<ISocketClient>();
    ...
    private async UniTask ConnectToServerAsync()
    {
        ...
        {
            host = serverDefine.Dev.Host;
            port = serverDefine.Dev.Port;

            await Client.Connect(host, port);
        }
    }

    private async UniTask DoLoadServerSetting()
    {
        var rq = Resources.LoadAsync<ServerDefine>("ServerDefine/TCPServerDefine");
        await rq;
        if (rq.asset != null)
        {
    ...
    private async UniTask OnLoginClicked()
    {
        ...
        if (kq)
        {
            if (Profile.AccountType == TypeOfAccount.JUST_REGISTERED)
            {
                await ShowOtpLoginPopUp();
            }
            ucs.TrySetResult();
        }
        else
    ...
    private async UniTask ShowOtpPopUp()
    {
       
        var uiConfirm = await UILoader.Instantiate<UIPopupOTP>(token: cts.Token);
        bool wait = true;
        uiConfirm.SetEntity(new UIPopupOTP.Entity()
        {
            
            confirmBtn = new ButtonEntity(() => 
            { 
                var otp = uiConfirm.inputOtp.text;
                
            }),
            resendOtpBtn = new ButtonEntity(() =>
            {
                var email = uiLogin.register.inputEmail.text;
                Debug.Log(email);
                
            }),
            closeBtn = new ButtonEntity(() => { wait = false; })
    ...
    private async UniTask ShowOtpLoginPopUp()
    {
        ...
        uiConfirm.SetEntity(new UIPopupOTP.Entity()
        {
            ...
            resendOtpBtn = new ButtonEntity(() =>
            {
                var email = uiLogin.login.inputName.text;
                Debug.Log(email);
